...
const templateTheory = `import React from 'react';
...
export default function Lektion__NUM__VocabTheory({ playAudio, playingId }: Props) {
  const lesson = VOCABULARY_DATA.find(l => l.id === 'l__NUM__');
        ...
        {Object.entries(categories).map(([title, words], idx) => (
          <div key={idx} className="bg-white/80 backdrop-blur-md rounded-3xl p-6 shadow-sm border-2 border-white">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {words.map((w, wIdx) => {
                const audioId = 'l__NUM__-vocab-' + idx + '-' + wIdx;
...
const templateEx = `import React from 'react';
...
export default function Lektion__NUM__VocabEx({ playAudio, playingId }: Props) {
  const lesson = VOCABULARY_DATA.find(l => l.id === 'l__NUM__');
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-4xl mx-auto">
      <div className="text-center space-y-4 mb-6">
        <h2 className="text-3xl font-black text-theme-dark uppercase tracking-tight">Luyện Tập Từ Vựng Lektion __NUM__</h2>
...
[21, 22, 23, 24].forEach(num => {
  const theoryCode = templateTheory.replace(/__NUM__/g, num);
  const exCode = templateEx.replace(/__NUM__/g, num);
  
  fs.writeFileSync('src/components/Lektion' + num + 'VocabTheory.tsx', theoryCode);
  fs.writeFileSync('src/components/Lektion' + num + 'VocabEx.tsx', exCode);
});
