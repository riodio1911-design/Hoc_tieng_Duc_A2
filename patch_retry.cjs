...
4. Update the 'id' to append '_s' and '_p' for split nouns to avoid duplication. For words that don't split, you can keep the original id or add nothing.
...
`;
  ...
  while(retries > 0) {
    try {
      ...
      console.error(`Error on \${lektionId}, retrying... \${retries} left`);
      retries--;
      await delay(5000 * (6 - retries)); // exponential backoff
    }
  }
...
async function main() {
  ...
  const lektionenToRefactor = ["l15", "l16", "l17", "l18", "l20"];
