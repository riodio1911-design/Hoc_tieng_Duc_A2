...
export default function Lektion19GrammarEx({ playAudio }: { playAudio?: (t: string, id: string, l?: string) => void }) {
  ...
  const questions = [
    {
      question: "Ich komme gerade ____ Theater.",
      options: ["aus dem", "vom", "im"],
      correct: 0,
      explanation: "Theater (rạp hát) là một không gian kín, chúng ta đi ra từ bên trong nó nên dùng 'aus dem'."
    },
    {
      question: "Herr Müller kommt in einer halben Stunde ____ Essen zurück.",
      options: ["aus dem", "vom", "zum"],
      correct: 1,
      explanation: "Essen (bữa ăn) là một sự kiện/hoạt động, nên dùng 'vom' (von dem)."
    },
    {
      question: "Sollen wir heute Abend ____ Kino gehen?",
      options: ["im", "zum", "ins"],
      correct: 2,
      explanation: "Đi vào bên trong rạp chiếu phim (hướng đến) dùng 'in' + Akkusativ = 'ins' (in das)."
    },
    {
      question: "Morgen gehe ich ____ Tante Thea.",
      options: ["zu", "nach", "in die"],
      correct: 0,
      explanation: "Đi đến chỗ một người dùng 'zu' (zu + Tên người không cần mạo từ, vì vậy chỉ là 'zu Tante Thea')."
    },
    {
      question: "Jana kommt gerade ____ Arzt.",
      options: ["aus dem", "vom", "beim"],
      correct: 1,
      explanation: "Từ chỗ người (bác sĩ) đi về dùng 'von' + Dativ = 'vom'."
    }
  ];
  ...
  const handleAnswer = (index: number) => {
    ...
    if (correct) {
      setScore(s => s + 1);
      if (playAudio) playAudio("Richtig", "correct-g19", "de-DE");
    } else {
      if (playAudio) playAudio("Falsch", "wrong-g19", "de-DE");
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-8">
        ...
        <p className="text-theme-dark/70 font-medium">Chọn câu trả lời đúng nhất (Woher / Wohin).</p>
      </div>
