...
export default function Lektion21VocabEx({ playAudio, playingId }: Props) {
  const lesson = VOCABULARY_DATA.find(l => l.id === 'l21');
  const vocabItems = lesson?.items.filter(item => !item.id.includes('g') && item.word) || [];

  return (
    <div className="space-y-8 animate-fade-in max-w-4xl mx-auto">
      <div className="text-center space-y-4 mb-6">
        <h2 className="text-3xl font-black text-theme-dark uppercase tracking-tight">Luyện Tập Từ Vựng Lektion 21</h2>
