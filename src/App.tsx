...
const MODULE_DATA = [
  {
    id: 1,
    title: "Modul 1",
    label: "Berufe, Familie, Wohnen, Tourismus",
    bg: "bg-emerald-500",
  ...
  },
  {
    id: 3,
    title: "Modul 3",
    label: "Sport, Gesundheit, Arbeitsleben",
    bg: "bg-cyan-600",
    color: "text-cyan-700",
  ...
  },
  {
    id: 4,
    title: "Modul 4",
    label: "Restaurant, Firmenporträt, Ernährung",
    bg: "bg-blue-500",
    color: "text-blue-600",
  ...
  },
  {
    id: 6,
    title: "Modul 6",
    label: "Informationen, Klima, Wetter",
    bg: "bg-violet-500",
...
const EFFECTS = [
  ...
  { id: "cheerful", name: "Vui vẻ", prompt: "Say cheerfully" },
  {
    id: "serious",
    name: "Trang trọng",
    prompt: "Say professionally and seriously",
  },
...
export default function App() {
  ...
  const downloadGrammarIntro = async (lessonId: string, script: string) => {
    ...
    try {
      const effectPrompt =
        EFFECTS.find((e) => e.id === voiceEffect)?.prompt || "Say clearly";
        ...
        contents: [
          {
            parts: [
              {
                text: `${effectPrompt}. Please read the following text naturally. The main language is Vietnamese, but make sure to pronounce any German words correctly in German: ${L23_INTRO_SCRIPT}`,
              },
            ],
          },
        ],
  ...
  const downloadL24Intro = async () => {
    ...
    try {
      ...
      const effectPrompt =
        voiceEffect === "gentle"
          ? "Use a very gentle, calm, and soothing tone"
          : voiceEffect === "energetic"
            ? "Use a highly energetic, enthusiastic, and fast-paced tone"
            ...
            contents: [
              {
                parts: [
                  {
                    text: `${effectPrompt}. Please read the following text naturally. The main language is Vietnamese, but make sure to pronounce any German words correctly in German: ${L24_INTRO_SCRIPT}`,
                  },
                ],
              },
            ],
      ...
      const base64Audio = data.candidates?.[0]?.content?.parts?.find((p: any) =>
  ...
  const downloadAllAudioForLesson = async () => {
    ...
    let speakingItems: any[] = [];
    if (SPEAKING_DATA[selectedLesson.id]) {
      const sData = SPEAKING_DATA[selectedLesson.id];
      const allSItems = [
        ...(sData.words || []),
        ...(sData.sentences || []),
        ...(sData.conversations || [])
      ];
      // Map 'de' to 'word' so the rest of the loop works
      speakingItems = allSItems.map(item => ({ ...item, word: item.de }));
    }

    // Deduplicate array of texts across all sources
    const uniqueItems = new Map<string, { text: string, lang: string, voice?: string }>();

    // 1. From selectedLesson.items/grammar (strip parentheses)
    const rawItems = [
      ...(selectedLesson.items || []),
      ...(selectedLesson.grammar || []),
    ];
    rawItems.forEach(item => {
      const t = item.word.split("(")[0].trim();
      uniqueItems.set(`de-DE:${t}`, { text: t, lang: 'de-DE' });
    });

    // 2. From speaking items
    speakingItems.forEach(item => {
      const t = item.word.split("(")[0].trim();
      uniqueItems.set(`de-DE:${t}`, { text: t, lang: 'de-DE' });
    });

    // 3. From generated manifest
    if (AUDIO_MANIFEST[selectedLesson.id]) {
      AUDIO_MANIFEST[selectedLesson.id].forEach(item => {
         const t = item.lang === 'de-DE' ? item.text.split("(")[0].trim() : item.text.trim();
         uniqueItems.set(`${item.lang}:${t}:${item.voice||''}`, { text: t, lang: item.lang, voice: item.voice });
      });
    }

    const itemsToDownload = Array.from(uniqueItems.values());
    ...
    for (let i = 0; i < itemsToDownload.length; i++) {
      ...
      const targetVoice = item.voice || voiceName;
      const cacheKey = `global-${voiceEffect}-${text}-${item.lang}-v5`;

      try {
        const cache = await caches.open("german-tts-cache-v1");
        const offlineKey = `/tts/${targetVoice}/${encodeURIComponent(cacheKey)}`;
        const cachedResponse = await cache.match(offlineKey);
  ...
  const startRecording = async (id: string, targetText: string) => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      setMicError(
        "Trình duyệt của bạn không hỗ trợ ghi âm hoặc đang chạy trong môi trường không bảo mật (HTTP).",
      );
      return;
    }
    ...
    try {
      ...
      let message =
        "Không thể truy cập Micro. Vui lòng kiểm tra cài đặt hệ thống.";
      ...
      if (
        ...
        message =
          "Trình duyệt hoặc Hệ thống đã chặn quyền truy cập Micro. Vui lòng kiểm tra cài đặt quyền của trình duyệt cho trang web này.";
        setMicState("denied");
      } else if (
        ...
        message =
          "Không tìm thấy thiết bị Micro. Vui lòng kết nối Micro và thử lại.";
      } else if (
        error.name === "NotReadableError" ||
        error.name === "TrackStartError"
      ) {
        message =
          "Micro đang bị sử dụng bởi một ứng dụng khác. Vui lòng kiểm tra và thử lại.";
      }
  ...
  const evaluatePronunciation = async (
    ...
    const executeFastLocalGrading = () => {
      ...
      if (spoken === target) {
        ...
        suggestion =
          "Phát âm chuẩn xác! Khẩu hình và phát âm của bạn hoàn toàn khớp với từ gốc.";
      } else if (spoken.includes(target) || target.includes(spoken)) {
        score = 80 + Math.floor(Math.random() * 9); // 80-88
        suggestion =
          "Đọc khá tốt, đúng âm chính nhưng cần chú ý ngắt nghỉ và các từ nối hoặc âm đuôi.";
      } else {
        const targetWords = target.split(" ");
        const spokenWords = spoken.split(" ");
        const matches = targetWords.filter((w) =>
        ...
        if (score < 40)
          suggestion =
            "Sai đoạn dài hoặc đọc không rõ chữ. Chú ý các từ bị thiếu. Hãy nghe lại audio mẫu.";
        else if (score < 70)
          suggestion =
            "Phát âm thiếu chính xác (có thể rơi rớt phụ âm cuối, hoặc sai nguyên âm umlaut).";
        else
          suggestion =
            "Gần đúng rồi, hãy chú ý các phụ âm phụ (st, en, er,...).";
      }
    ...
    if (
      ...
      offlineTranscriptRef.current.trim().length > 0
    ) {
...
Return ONLY JSON: {"score": 85, "transcription": "...", "suggestion": "precise tip in VNese"}`;
          ...
          if (!res.ok) {
            ...
            try {
              ...
              throw new Error(
                "Unexpected server response, possibly missing API Key or misconfigured server.",
              );
            }
            throw new Error(errMsg);
          }
  ...
  return (
    <div className="min-h-screen pb-20 relative overflow-x-hidden">
      ...
      <div
        className="fixed bottom-20 -right-20 w-80 h-80 bg-theme-secondary/10 rounded-full blur-[100px] pointer-events-none -z-10 animate-pulse"
      ...
      <header className="sticky top-0 z-40 bg-white/70 backdrop-blur-xl px-6 py-4 flex items-center justify-between border-b-2 border-theme-dark/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-black text-xl tracking-tight text-theme-dark">
              Audio Deutsch
            </h1>
            <p className="text-[10px] text-theme-primary font-black uppercase tracking-[0.15em]">
              Luyện Nghe & Phát Âm
            </p>
          </div>
        </div>
          ...
          {!selectedLesson ? (
            ...
            >
              <div className="mb-10 text-center space-y-2">
                <h2 className="text-4xl font-display font-black tracking-tight text-theme-dark">
                  Chọn bài học để bắt đầu
                </h2>
                <p className="text-theme-dark/90 font-bold">
                  Khám phá các mẫu câu thông dụng theo từng chủ đề thực tế.
                </p>
              </div>
              ...
              <div className="space-y-8">
                ...
                >
                  ...
                  <div className="flex-1 text-center md:text-left z-10">
                    <h3 className="font-display font-black text-3xl md:text-4xl text-theme-primary transition-colors mb-2">
                      Bài Ôn tập giáo trình Menschen A2
                    </h3>
                    <p className="text-base md:text-lg text-theme-dark/80 font-bold max-w-2xl">
                      Phòng luyện giao tiếp AI tương tác trực tiếp & Trạm ôn tập
                      từ vựng, ngữ pháp tổng hợp hàng ngày (Spaced Repetition).
                    </p>
              ...
              <div className="mt-12 bg-white rounded-3xl p-6 sm:p-8 shadow-xl shadow-indigo-100/50 border border-theme-dark/10 flex flex-col md:flex-row gap-6 items-start">
                ...
                <div className="space-y-4 flex-1">
                  <h4 className="text-lg font-black text-theme-dark">
                    Hướng dẫn kích hoạt tính năng "Luyện nói Offline" (Không cần
                    Wifi)
                  </h4>
                  <p className="text-theme-dark/80 font-medium leading-relaxed">
                    Mặc định, ứng dụng cần có mạng Wifi/4G để tải các bài học và
                    sử dụng AI chấm điểm giọng nói. Tuy nhiên, nếu bạn muốn dùng
                    App{" "}
                    <b>
                      ngay cả khi đi máy bay hoặc đến nơi mất sóng (Offline
                      Version)
                    </b>{" "}
                    với độ trễ phản hồi cực nhanh (0s), bạn chỉ cần làm theo
                    hướng dẫn sau đúng 1 lần:
                  </p>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-zinc-50 border border-zinc-200 p-5 rounded-2xl text-[13px] font-medium text-zinc-700 leading-relaxed shadow-sm">
                      <strong className="text-theme-dark text-sm block mb-2 flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-theme-primary"></div>{" "}
                        Dành cho iPhone / iPad (iOS):
                      </strong>
                      <p className="mb-1 text-slate-800 font-bold">
                        Để máy chấm điểm được Offline (Nhận diện giọng nói):
                      </p>
                      <ul className="list-decimal pl-4 mb-3 space-y-1 text-zinc-600">
                        <li>
                          Vào{" "}
                          <b className="text-theme-primary">
                            Cài đặt (Settings)
                          </b>{" "}
                          {">"}{" "}
                          <b className="text-theme-primary">
                        ...
                        <li>
                          Mở phần{" "}
                          <b className="text-theme-primary">
                            Ngôn ngữ Đọc chính tả
                          </b>{" "}
                          {">"} Đánh dấu tích để tải về{" "}
                          <b className="text-theme-primary">Tiếng Đức (Đức)</b>.
                          <br />
                          <i className="text-xs text-rose-500">
                            *Nếu không thấy Tiếng Đức, quay lại mục Bàn phím{" "}
                            {">"} Thêm bàn phím mới {">"} Chọn Tiếng Đức.
                          </i>
                        </li>
                      </ul>
                      <p className="mb-1 text-slate-800 font-bold">
                        Để nghe Audio offline (Tùy chọn):
                      </p>
                      <ul className="list-decimal pl-4 space-y-1 text-zinc-600">
                        <li>
                          Vào <b className="text-theme-primary">Cài đặt</b>{" "}
                          {">"}{" "}
                          <b className="text-theme-primary">
                        ...
                        <li>
                          Chọn{" "}
                          <b className="text-theme-primary">
                            Đọc & Nói (Read & Speak / Spoken Content)
                          </b>{" "}
                          {">"}{" "}
                          <b className="text-theme-primary">
                            Giọng nói (Voices)
                        ...
                        <li>
                          Tìm <b className="text-theme-primary">Tiếng Đức</b> và
                          bấm biểu tượng "Đám mây" tải về một giọng bạn thích.
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="bg-zinc-50 border border-zinc-200 p-5 rounded-2xl text-[13px] font-medium text-zinc-700 leading-relaxed shadow-sm">
                      <strong className="text-theme-dark text-sm block mb-2 flex items-center gap-2">
                        ...
                        Dành cho Android (Samsung, Xiaomi...):
                      </strong>
                      <ul className="list-decimal pl-4 space-y-2 text-zinc-600">
                        ...
                        <li>
                          Tìm kiếm từ khoá{" "}
                          <b className="text-theme-primary">"Đọc chính tả"</b>{" "}
                          (Voice typing) hoặc{" "}
                          <b className="text-theme-primary">
                            "Văn bản thành giọng nói"
                          </b>{" "}
                          (Text-to-speech) qua thanh tìm kiếm.
                        </li>
                        <li>
                          Trong mục quản lý ngôn ngữ của Google, chọn Tải xuống
                          (Download) gói{" "}
                          <b className="text-theme-primary">
                        ...
                        <li>
                          <i>
                            *Lưu ý: Tuỳ từng hãng máy mà đường dẫn có thể nằm
                            trong mục "Bàn phím" hoặc "Ngôn ngữ {">"} Hệ thống".
                          </i>
                        </li>
                      </ul>
                    </div>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="space-y-2">
                  <h2 className="text-4xl font-display font-black text-theme-dark flex max-md:flex-col max-md:items-start items-center gap-4">
                    ...
                    {selectedLesson.id === "l21" && (
                      <div className="flex items-center gap-2 bg-theme-primary/10 pl-1 pr-4 py-1 rounded-full border border-theme-primary/20">
                        <button
                          onClick={() =>
                            playAudio(L21_INTRO_SCRIPT, "l21-intro", "vi-VN")
                          }
                    ...
                    {selectedLesson.id === "l22" && (
                      <div className="flex items-center gap-2 bg-theme-primary/10 pl-1 pr-4 py-1 rounded-full border border-theme-primary/20">
                        <button
                          onClick={() =>
                            playAudio(L22_INTRO_SCRIPT, "l22-intro", "vi-VN")
                          }
                    ...
                    {selectedLesson.id === "l23" && (
                      <div className="flex items-center gap-2 bg-theme-primary/10 pl-1 pr-4 py-1 rounded-full border border-theme-primary/20">
                        <button
                          onClick={() =>
                            playAudio(L23_INTRO_SCRIPT, "l23-intro", "vi-VN")
                          }
                    ...
                    {selectedLesson.id === "l24" && (
                      <div className="flex items-center gap-2 bg-theme-primary/10 pl-1 pr-4 py-1 rounded-full border border-theme-primary/20">
                        <button
                          onClick={() =>
                            playAudio(L24_INTRO_SCRIPT, "l24-intro", "vi-VN")
                          }
                  ...
                  <p className="text-theme-dark/80 font-black text-sm uppercase tracking-widest">
                  ...
                  {downloadProgress ? (
                    <div className="w-48 bg-zinc-50 border border-zinc-100 rounded-xl p-3 shadow-sm">
                      <div className="flex justify-between text-[10px] font-black text-theme-dark/60 uppercase mb-2">
                        <span>
                          Đang tải: {downloadProgress.current}/
              ...
              <div className="space-y-4 pb-20">
                <Suspense fallback={<div className="p-8 text-center text-theme-dark/50 animate-pulse">Đang tải nội dung...</div>}>
                  ...
                  <div className="space-y-6">
                    ...
                    {selectedLesson.id === "l15" &&
                    activeSubTab === "mediagame" ? (
                      <Lektion15MediaGame
                        playAudio={playAudio}
                        playingId={playingId}
                      />
                    ) : selectedLesson.id === "l16" &&
                      activeSubTab === "hotelgame" ? (
                      <Lektion16HotelGame
                        playAudio={playAudio}
                        playingId={playingId}
                      />
                    ) : selectedLesson.id === "l17" &&
                      activeSubTab === "travelgame" ? (
                      <Lektion17TravelGame />
                    ) : selectedLesson.id === "l18" &&
                      activeSubTab === "travelgame" ? (
                      ...
                      activeSubTab === "wordguess" ? (
                      <Lektion24WordGuessingGame
                        playAudio={playAudio}
                        playingId={playingId}
                      />
                    ) : selectedLesson.id === "l24" &&
                      activeSubTab === "dachquiz" ? (
                      <Lektion24DACHQuiz
                        playAudio={playAudio}
                        playingId={playingId}
                      />
                  ...
                  <div>
                    {(selectedLesson.id === "l1" ||
                      selectedLesson.id === "l2" ||
                      selectedLesson.id === "l3" ||
                      selectedLesson.id === "l4" ||
                      selectedLesson.id === "l5" ||
                      selectedLesson.id === "l6" ||
                      selectedLesson.id === "l7" ||
                      selectedLesson.id === "l8" ||
                      selectedLesson.id === "l9" ||
                      selectedLesson.id === "l10" ||
                      selectedLesson.id === "l11" ||
                      selectedLesson.id === "l12" ||
                      selectedLesson.id === "l13" ||
                      selectedLesson.id === "l14" ||
                      selectedLesson.id === "l15" ||
                      selectedLesson.id === "l16" ||
                      selectedLesson.id === "l17" ||
                      selectedLesson.id === "l18" ||
                    ...
                    {selectedLesson.id === "l1" && activeSubTab === "theory" ? (
                      ...
                      activeSubTab === "theory" ? (
                      <Lektion10VocabTheory
                        lessonItems={selectedLesson.items}
                        playAudio={playAudio}
                      />
                    ) : selectedLesson.id === "l11" &&
                      activeSubTab === "theory" ? (
                      <Lektion11VocabTheory
                        lessonItems={selectedLesson.items}
                        playAudio={playAudio}
                      />
                    ) : selectedLesson.id === "l12" &&
                      activeSubTab === "theory" ? (
                      <Lektion12VocabTheory
                        vocabItems={selectedLesson.items}
                        playAudio={playAudio}
                      />
                    ) : selectedLesson.id === "l13" &&
                      activeSubTab === "theory" ? (
                      <Lektion13VocabTheory
                        playAudio={playAudio}
                        playingId={playingId}
                      />
                    ) : selectedLesson.id === "l14" &&
                      activeSubTab === "theory" ? (
                      <Lektion14VocabTheory
                        playAudio={playAudio}
                        playingId={playingId}
                      />
                    ) : selectedLesson.id === "l17" &&
                      activeSubTab === "theory" ? (
                      <Lektion17VocabTheory
                        playAudio={playAudio}
                        playingId={playingId}
                      />
                    ) : selectedLesson.id === "l18" &&
                      activeSubTab === "theory" ? (
                      <Lektion18VocabTheory
                        playAudio={playAudio}
                        playingId={playingId}
                      />
                    ) : selectedLesson.id === "l19" &&
                      activeSubTab === "theory" ? (
                      <Lektion19VocabTheory
                        playAudio={playAudio}
                        playingId={playingId}
                      />
                      ...
                      activeSubTab === "exercises" ? (
                      <Lektion11VocabEx playAudio={playAudio} />
                    ) : selectedLesson.id === "l12" &&
                      activeSubTab === "exercises" ? (
                      <Lektion12VocabEx vocabItems={selectedLesson.items} playAudio={playAudio} />
                    ) : selectedLesson.id === "l13" &&
                      activeSubTab === "exercises" ? (
                      <Lektion13VocabEx
                        playAudio={playAudio}
                  ...
                  <div>
                        ...
                        <div className="bg-theme-cream/50 p-1.5 rounded-2xl flex flex-wrap justify-center gap-1 shadow-inner max-w-full">
                          ...
                          >
                            {selectedLesson.id === "l22"
                              ...
                              : selectedLesson.id === "l23"
                                ? "Bài tập Ngữ pháp (15 Phần)"
                                : selectedLesson.id === "l24"
                                  ? "Bài tập Ngữ pháp (8 Phần)"
                                  : selectedLesson.id === "l18"
                                    ? "Bài tập & Test Ngữ pháp"
                                    : selectedLesson.id === "l19"
                                      ? "Bài tập Ngữ pháp Woher/Wohin"
                                      : selectedLesson.id === "l20"
                                        ? "Bài tập Präteritum"
                                        : selectedLesson.id === "l17"
                                          ? "Bài tập & Test Ngữ pháp"
                                          : selectedLesson.id === "l16"
                                            ...
                                            : ["l14", "l15"].includes(
                                              ...
                                              ? "Bài tập Passiv"
                                              : selectedLesson.id === "l13"
                                                ? "Bài tập Ngữ pháp (Als & Wenn)"
                                                : selectedLesson.id === "l12"
                                                  ? "Bài tập Ngữ pháp (wenn)"
                                                  : selectedLesson.id === "l11"
                                                    ? "Bài tập Ngữ pháp (Phản thân)"
                                                    : selectedLesson.id ===
                                                        "l10"
                                                      ? "Bài tập Ngữ pháp (dass)"
                                                      : selectedLesson.id ===
                                                          "l9"
                                                        ? "Bài tập Ngữ pháp (So sánh)"
                                                        : selectedLesson.id ===
                                                            "l8"
                                                          ? "Bài tập Ngữ pháp (weil, deshalb)"
                                                          : selectedLesson.id ===
                                                              "l7"
                                                            ? "Bài tập Ngữ pháp (Câu mệnh lệnh)"
                                                            : selectedLesson.id ===
                                                                "l6"
                                                              ? "Bài tập Ngữ pháp (Giới từ t/g)"
                                                              : selectedLesson.id ===
                                                                  "l5"
                                                                ? "Bài tập Ngữ pháp (Tính từ)"
                                                                : selectedLesson.id ===
                                                                    "l4"
                                                                  ? "Bài tập Ngữ pháp (Adjektive)"
                                                                  : selectedLesson.id ===
                                                                      "l3"
                                                                    ? "Bài tập Ngữ pháp (-ung, -er)"
                                                                    : selectedLesson.id ===
                                                                        "l2"
                                                                      ? "Bài tập Ngữ pháp (Chỉ vị trí)"
                                                                      : selectedLesson.id ===
                                                                          "l1"
                                                                        ? "Bài tập Ngữ pháp (Sở hữu, Quá khứ)"
                                                                        : "Bài tập Ngữ pháp"}
                          </button>
                        </div>
                      </div>
                    ...
                    grammarSubTab === "exercises" ? (
                      ...
                      grammarSubTab === "exercises" ? (
                      <Lektion11GrammarEx playAudio={playAudio} />
                    ) : selectedLesson.id === "l11" &&
                      grammarSubTab === "theory" ? (
                      <Lektion11GrammarTheory playAudio={playAudio} />
                    ) : selectedLesson.id === "l12" &&
                      grammarSubTab === "exercises" ? (
                      <Lektion12GrammarEx playAudio={playAudio} />
                    ) : selectedLesson.id === "l12" &&
                      grammarSubTab === "theory" ? (
                      <Lektion12GrammarTheory playAudio={playAudio} />
                    ) : selectedLesson.id === "l13" &&
                      grammarSubTab === "exercises" ? (
                      <Lektion13GrammarEx
                        playAudio={playAudio}
                                ...
                                playAudio(
                                  selectedLesson.id === "l23"
                                    ...
                                    : selectedLesson.id === "l24"
                                      ? L24_GRAMMAR_SCRIPT
                                      : L22_GRAMMAR_SCRIPT,
                                  `${selectedLesson.id}_grammar_intro`,
                                  "vi-VN",
                                )
                              }
            ...
            >
              ...
              <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                ...
                <div className="space-y-3">
                  <p className="text-xs font-bold text-zinc-800 uppercase tracking-widest border-b border-zinc-100 pb-1">
                    Loại giọng
                  </p>
                  <div className="flex p-1 bg-zinc-100 rounded-2xl gap-1">
                    <button
                      onClick={() => {
                        ...
                        if (
                          !VOICES.find(
                            (v) => v.id === voiceName && v.gender === "Female",
                          )
                        ) {
                          setVoiceName("Aoede");
                        }
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (
                          !VOICES.find(
                            (v) => v.id === voiceName && v.gender === "Male",
                          )
                        ) {
                          setVoiceName("Fenrir");
                        }
