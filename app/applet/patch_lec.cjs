...
for (let i = 1; i <= 12; i++) {
  const file = path.join(__dirname, `src/components/Lektion${i}Slides.tsx`);
  ...
  if (!content.includes("const [activeSlideTab, setActiveSlideTab] = useState")) {
    content = content.replace(
      /const \[currentSlide, setCurrentSlide\] = useState\(-1\);/g,
  ...
  if (!content.includes('setActiveSlideTab(\'roadmap\')')) {
    ...
    // We can just replace `<div className="grid md:grid-cols-2`
    ...
    // Wait, let's find `      case 0:` and step backward to replace the ending elements.
    // The previous code ends like this:
    //             </div>
    //           </div>
    //         );
    //       case 0:
    // We replace the last two `</div>` before `); \n case 0:`
