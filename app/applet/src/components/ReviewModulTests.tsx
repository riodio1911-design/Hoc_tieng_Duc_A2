...
export default function ReviewModulTests() {
  ...
  const startTest = (moduleId: number) => {
    ...
    selectedGrammars.forEach((g, index) => {
        generatedQs.push({
            id: `g-${index}`,
            type: 'grammar',
            question: `${g.word} (${g.meaning})\n${g.rule}`,
            options: ['Đúng', 'Hiểu, chuyển tiếp', 'Rõ ràng', 'Ghi nhớ'],
            correctAnswer: 'Đúng',
            explanation: `${g.example} (${g.exampleTranslation})`
        });
    });

    // Make options for grammar more generic true/false since it's hard to gen good distractors
    const finalQs = shuffle(generatedQs).map((q, idx) => {
        if (q.type === 'grammar') {
            return {
                ...q,
               question: `Cấu trúc: "${g.word}" có nghĩa là gì?`,
               options: shuffle([...shuffle(allGrammar.filter(i => i.id !== g.id)).slice(0, 3).map(i => i.meaning), g.meaning]),
    ...
    // Let's rewrite grammar to use proper meanings
    ...
    return (
      <div className="space-y-6">
        <div className="bg-sky-50 border-2 border-sky-200 p-6 rounded-2xl">
           ...
           <p className="text-theme-dark/80 font-medium">Làm bài test tổng hợp (10 câu) cho từng Modul trong sách Menschen A2. Bao gồm từ vựng và ngữ pháp để củng cố kiến thức trước khi chuyển sang Modul mới.</p>
        </div>
  ...
  if (showResult) {
    return (
      <div className="bg-white rounded-[32px] p-8 md:p-12 text-center border-4 border-theme-dark/5 shadow-xl max-w-2xl mx-auto">
