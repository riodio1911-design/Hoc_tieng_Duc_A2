...
import { VOCABULARY_DATA } from './src/constants.js';
...
for (const file of files) {
  if (file.startsWith('Lektion') && file.endsWith('.tsx')) {
    ...
    const wordRegex = /word:\s*["']([^"']+)["'],\s*(?:ipa:\s*["']([^"']+)["'],\s*)?meaning:\s*["']([^"']+)["'](?:,\s*example:\s*["']([^"']+)["'])?/g;
    ...
    // some grammar might be in { w: "...", t: "..." } or something, but the user specifically asked for "các từ ngữ pháp trong các bài học" and "từ vựng (số nhiều số ít)"
  }
}
